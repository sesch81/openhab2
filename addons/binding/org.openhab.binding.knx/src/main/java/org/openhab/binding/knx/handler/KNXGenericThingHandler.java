...
public class KNXGenericThingHandler extends BaseThingHandler
    ...
    public KNXGenericThingHandler(Thing thing, ItemChannelLinkRegistry itemChannelLinkRegistry) {
        super(thing);
    }
    ...
    private void scheduleReadJob(GroupAddress groupAddress, String dpt, boolean mustRead, BigDecimal readInterval) {
        ...
        if (mustRead || (readInterval != null && readInterval.intValue() > 0)) {
            readRunnable = new ReadRunnable(groupAddress, dpt);
        }

        if (readRunnable != null) {
            if (readInterval != null && readInterval.intValue() > 0) {
                readFutures.add(
                        scheduler.scheduleWithFixedDelay(readRunnable, 0, readInterval.intValue(), TimeUnit.SECONDS));
            } else if (mustRead) {
                scheduler.schedule(readRunnable, 0, TimeUnit.SECONDS);
            }
        }
    }
    ...
    public void handleCommand(ChannelUID channelUID, Command command) {
        ...
        if (command instanceof RefreshType) {
            ...
            if (channelConfiguration.get(AUTO_UPDATE) != null && (boolean) channelConfiguration.get(AUTO_UPDATE)
                ...
                logger.trace("Adding {} to the autoUpdateChannels", channelUID);
                autoUpdateChannels.add(channelUID);
                logger.trace("updateState {} {}", channelUID, command);
