...
@XmlType(name = "HeliosEvent", namespace = HeliosHandler27.HELIOS_URI, propOrder = {
	...
	public boolean equals(Object obj) {
		...
		if (event.getEventName().equals(eventName)
			...
			if (data instanceof SOAPKeyPressed) {
				return ((SOAPKeyPressed) data).getKeyCode().equals(
						((SOAPKeyPressed) eventData).getKeyCode());
			}

			if (data instanceof SOAPCallStateChanged) {
				return ((SOAPCallStateChanged) data).getDirection().equals(
						((SOAPCallStateChanged) eventData).getDirection())
						&& ((SOAPCallStateChanged) data).getState()
								.equals(((SOAPCallStateChanged) eventData)
										.getState());
			}

			if (data instanceof SOAPCardEntered) {
				return ((SOAPCardEntered) data).getCard().equals(
						((SOAPCardEntered) eventData).getCard())
						&& ((SOAPCardEntered) data).getValid().equals(
								((SOAPCardEntered) eventData).getValid());
			}

			if (data instanceof SOAPCodeEntered) {
				return ((SOAPCodeEntered) data).getCode().equals(
						((SOAPCodeEntered) eventData).getCode())
						&& ((SOAPCodeEntered) data).getValid().equals(
								((SOAPCodeEntered) eventData).getValid());
			}

			if (data instanceof SOAPDeviceState) {
				return ((SOAPDeviceState) data).getState().equals(
						((SOAPDeviceState) eventData).getState());
			}
