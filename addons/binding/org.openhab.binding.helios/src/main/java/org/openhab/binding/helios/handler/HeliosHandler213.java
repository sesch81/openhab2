...
import org.openhab.binding.helios.ws.rest.RESTPort;
import org.openhab.binding.helios.ws.rest.RESTSubscribeResponse;
import org.openhab.binding.helios.ws.rest.RESTSwitch;
import org.openhab.binding.helios.ws.rest.RESTSystemInfo;
...
public class HeliosHandler213 extends BaseThingHandler {
    ...
    private Logger logger = LoggerFactory.getLogger(HeliosHandler213.class);
    ...
    protected Client heliosClient;
    WebTarget baseTarget;
    WebTarget systemTarget;
    WebTarget logTarget;
    WebTarget switchTarget;
    WebTarget portTarget;
    String ipAddress;
    ...
    protected ScheduledFuture<?> logJob;
    private static final long POLLING_RETRY_INTERVAL = 5;
    ...
    // TODO : Reconnect job in case of problems
    ...
    public void initialize() {
        ...
        if (ipAddress != null && !ipAddress.isEmpty() && username != null && !username.isEmpty() && password != null
            ...
            try {
                ...
                logger.error("An exception occurred while initialising the SSL context : '{}'", e1.getMessage());
            }
        ...
        if (getThing().getStatus() == ThingStatus.ONLINE) {
            ...
            if (response != null) {
                ...
                if (jsonObject.get("success").toString().equals("true")) {
                    ...
                    logger.info("The subscription id to pull logs from the Helios IP Vario '{}' is '{}'",
                ...
                logger.warn("An error occurred while subscribing to the log entries of the Helios IP Vario '{}'",
                        getThing().getUID().toString());
                updateStatus(ThingStatus.OFFLINE, ThingStatusDetail.CONFIGURATION_ERROR);
                scheduler.schedule(resetRunnable, RESET_INTERVAL, TimeUnit.SECONDS);
                return 0;
    ...
    private void unsubscribe() {
        ...
        if (getThing().getStatus() == ThingStatus.ONLINE) {
            ...
            if (response != null) {
                JsonObject jsonObject = parser.parse(response.readEntity(String.class)).getAsJsonObject();
                if (jsonObject.get("success").toString().equals("true")) {
                    logger.info("Successfully unsubscribed from the log entries of the Helios IP Vario '{}'",
                            getThing().getUID().toString());
                } else {
                    RESTError error = gson.fromJson(jsonObject.get("error").toString(), RESTError.class);
                    logger.error(
    ...
    private List<RESTSwitch> getSwitches() {
        ...
        if (response != null) {
            JsonObject jsonObject = parser.parse(response.readEntity(String.class)).getAsJsonObject();
            if (jsonObject.get("success").toString().equals("true")) {
                logger.info("Successfully requested switch capabilities from the Helios IP Vario '{}'",
                        getThing().getUID().toString());
            ...
            logger.warn("An error occurred while requesting switch capabilities from the Helios IP Vario '{}'",
                    getThing().getUID().toString());
            updateStatus(ThingStatus.OFFLINE, ThingStatusDetail.CONFIGURATION_ERROR);
            scheduler.schedule(resetRunnable, RESET_INTERVAL, TimeUnit.SECONDS);
            return null;
    ...
    private void triggerSwitch(String id) {
        if (getThing().getStatus() == ThingStatus.ONLINE) {
            ...
            try {
                ...
                logger.error("An exception occurred while triggering a switch  on the Helios IP Vario '{}' : '{}'",
                        getThing().getUID().toString(), e.getMessage());
                updateStatus(ThingStatus.OFFLINE, ThingStatusDetail.COMMUNICATION_ERROR);
                scheduler.schedule(resetRunnable, RESET_INTERVAL, TimeUnit.SECONDS);
                return;
            ...
            if (response != null) {
                JsonObject jsonObject = parser.parse(response.readEntity(String.class)).getAsJsonObject();
                if (jsonObject.get("success").toString().equals("true")) {
                    logger.info("Successfully triggered a switch on the Helios IP Vario '{}'",
                            getThing().getUID().toString());
                } else {
                    RESTError error = gson.fromJson(jsonObject.get("error").toString(), RESTError.class);
                    logger.error(
    ...
    private void enableSwitch(String id, boolean flag) {
        if (getThing().getStatus() == ThingStatus.ONLINE) {
            ...
            if (response != null) {
                JsonObject jsonObject = parser.parse(response.readEntity(String.class)).getAsJsonObject();
                if (jsonObject.get("success").toString().equals("true")) {
                    logger.info("Successfully dis/enabled a  switch on the Helios IP Vario '{}'",
                            getThing().getUID().toString());
                } else {
                    RESTError error = gson.fromJson(jsonObject.get("error").toString(), RESTError.class);
                    logger.error(
    ...
    private List<RESTPort> getPorts() {
        ...
        if (response != null) {
            JsonObject jsonObject = parser.parse(response.readEntity(String.class)).getAsJsonObject();
            if (jsonObject.get("success").toString().equals("true")) {
                logger.info("Successfully requested port capabilities from the Helios IP Vario '{}'",
                        getThing().getUID().toString());
                JsonObject js = (JsonObject) jsonObject.get("result");
            ...
            if (getThing().getStatus() == ThingStatus.ONLINE) {
                ...
                if (logSubscriptionID != 0) {
                    ...
                    for (RESTEvent event : events) {
                        ...
                        date.setTime(Long.valueOf(event.utcTime));
                        logger.debug("Received the event for Helios IP Vario '{}' with ID '{}' of type '{}' on {}",
                                new Object[] { getThing().getUID().toString(), event.id, event.event,
                        ...
                        switch (event.event) {
                            case DEVICESTATE: {
                                ...
                                DateTimeType stampType = new DateTimeType(DATE_FORMATTER.format(date));
                ...
                if (logJob != null) {
                    logJob = scheduler.schedule(logRunnable, 0, TimeUnit.SECONDS);
                }
            } else {
                if (logJob != null) {
                    logJob = scheduler.schedule(logRunnable, POLLING_RETRY_INTERVAL, TimeUnit.SECONDS);
                }
            }
        }
    };
